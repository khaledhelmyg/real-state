...
class Order{
    ...
    static getReceiptPdfName=async(req,res)=>{
        try {
            const order=await ordertModel.findById(req.params.id)
            // if(!order)throw new Error("inavlid order id")
            const user=await User.findById(order.userId)
            ...
            // res.sendFile(path.join(__dirname,`../${fileName}`), {"contenttype": 'application/pdf'}) 
            console.log(fileName)
            resHelper(res,200,true,fileName,'get pdf  File name')
        } catch (err) {
            resHelper(res,400,false,err,err.message)
        }
    }
